...
describe('renderString', () => {
    describe('renderString', () => {
        test('should render a string using default delimiters', () => {
            ...
            expect(renderString('Hello, {{ name }}!', { name: 'Joe' })).toBe('Hello, Joe!');
        });

        test('should render a string using custom delimiters', () => {
            expect(
                renderString('Hello, <% name %>!', { name: 'Joe' }, { delimiters: ['<%', '%>'] })
            ).toBe('Hello, Joe!');
            expect(
                renderString('Hello, ${{ name }}!', { name: 'Joe' }, { delimiters: ['${{', '}}'] })
            ).toBe('Hello, Joe!');
        ...
        test('should render a string using multiple filters', () => {
            expect(renderString('Hello, {{ name | upcase | downcase }}!', { name: 'Joe' })).toBe(
                'Hello, joe!'
            );
        });
        ...
        test('should return the actual value if there is just one expression', () => {
            ...
            expect(renderString('{{ value }} {{ value }}', { value: 123 })).toBe('123 123');
            expect(renderString('"{{ value }}"', { value: 123 })).toBe('"123"');
        ...
        test('should return ignore invalid filters', () => {
            expect(renderString('{{ value | invalidFilter }}', { value: 123 })).toBe(123);
        });
    });
});
